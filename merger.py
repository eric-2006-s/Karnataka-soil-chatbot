...
def add_boundary_layer(m, geojson_data, name_key, fill_color="#4caf50", border_color="#1a4d1a"):
    ...
    folium.GeoJson(
        geojson_data,
        style_function=lambda f: {"fillColor": fill_color, "color": border_color, "weight": 1.5, "fillOpacity": 0.25},
...
search_mode = st.radio(
    "Search by",
    ["District / Sub-district / Village (dropdown)", "Map (click to select)", "Latitude & Longitude"]
)
...
# ── Mode 2: Map-based drill-down picker (boundary polygons) ─────
    ...
    if st.session_state.map_level == "district":
        ...
        add_boundary_layer(m, gdf_to_geojson(district_gdf), "DISTRICT", fill_color="#fff59d", border_color="#f9a825")
